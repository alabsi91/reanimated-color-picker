...
export function RedSlider({
  ...
  const handleStyle = useAnimatedStyle(() => {
    const { r } = HSVA2RGBA(hueValue.value, saturationValue.value, brightnessValue.value);
  ...
  const onGestureUpdate = ({ x, y }: PanGestureHandlerEventPayload) => {
    ...
    const { h, s, v } = RGBA2HSVA(newRedValue, g, b);
  ...
  const greenBlue = useAnimatedStyle(() => {
    const { g, b } = HSVA2RGBA(hueValue.value, saturationValue.value, brightnessValue.value);
    if (isWeb) {
