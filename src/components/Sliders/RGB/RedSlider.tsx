...
export function RedSlider({
  ...
  const handleStyle = useAnimatedStyle(() => {
    const { r } = colorKit.runOnUI().RGB({ h: hueValue.value, s: saturationValue.value, v: brightnessValue.value }).object(false);

    const length = (vertical ? height.value : width.value) - (boundedThumb ? thumbSize : 0),
      percent = (r / 255) * length,
  ...
  const onGestureUpdate = ({ x, y }: PanGestureHandlerEventPayload) => {
    ...
    if (newRedValue === r) return;

    const { h, s, v } = colorKit.runOnUI().HSV({ r: newRedValue, g, b }).object(false);
  ...
  const greenBlue = useAnimatedStyle(() => {
    const { g, b } = colorKit.runOnUI().RGB({ h: hueValue.value, s: saturationValue.value, v: brightnessValue.value }).object();
    if (isWeb) {
